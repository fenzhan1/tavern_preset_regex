...
- **首次使用务必在 WebUI 点一次「保存预设」**：只有保存后 `setvar.json` 才会写入
  `mofox_order`。旧数据没有这个字段时会用默认顺序「系统提示词 → 用户上下文(历史)
  → 本轮新输入 → 预设 → Tool」，历史不会排到预设后面。
...
- 请求里的对话内容被拆成两条预设条目，可以像其他条目一样拖动：
  ...
  重排是**幂等**的：工具调用后的二次请求里已经带着上次注入的预设条目，插件会
  复用它们并重新按 `mofox_order` 排序，不会重复注入，也不会因为「已注入过」
  就跳过重排。
