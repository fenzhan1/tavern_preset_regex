...
- 「MoFox 用户上下文」这一块（历史消息 + 上一轮回复 + 工具调用记录 + 本轮新输入）
  的位置可在预设标签顶部的「对话块位置」里改：
  - `跟随顺序表`：按 `mofox_order` 里拖到的位置；
  - `严格按顺序表`：固定块保持在顺序表位置，顺序表里的 `mofox_user` 只代表
    「本轮新输入」，历史部分放在第一条预设之前；
  - `预设排在历史+本轮新输入之后`：`历史/上轮回复/工具调用 → 本轮新输入 → 预设条目`；
  - `头部预填充 + 预设排在最后`：`头部预填充预设 → 系统提示词 → 历史 → 本轮新输入
    → 其余预设条目 → 工具声明`（头部内容在「小说」标签的「头部预填充预设」里填）；
  - `紧跟系统提示词`：排在 system/tool 固定块之后、所有预设条目之前；
  - `固定放最后`：永远排在请求末尾。
